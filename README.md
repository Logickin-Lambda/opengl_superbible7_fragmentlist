# Update: after all the months using zig, I am wondering: Is the book shitty or my coding skills?
Superbible is not a good book to learn about OpenGl since it has all the references included in the book all in one go; for example, it introduces nearly all the buffer type in the buffer chapter yet the beginner don't really use all of them that lead to an overwhelming amount concepts. The hiding of details about the backend and the in-house library are also a problem since their framework is not used elsewhere professionally, but the library has no explanation with their decision of certain use of values and logic over the abstractions which is totally against the spirit of zig.

However, was my coding skill good? Since I have stopped doing the openGL stuff, during all the contributions, I have learnt anyopaque, loading both static and dynamic c libraries, and how to properly handle ZLS and the build system. Seems like it is not just the book being bad, but my coding skills shittier, but this is a good thing because now I have a better understanding to the language so that I can handle this series better than before. Thus, I guess in some point of time, I am going to rewrite this series with the new zig knowledge I have gained, and continue to work on other chapters.
